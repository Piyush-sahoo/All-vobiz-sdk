package ai.vobiz.infrastructure
...
open class ApiClient(val baseUrl: String, val client: Call.Factory = defaultClient) {
    companion object {
        ...
        const val baseUrlKey: String = "ai.vobiz.baseUrl"
    ...
    protected fun <T> updateAuthParams(requestConfig: RequestConfig<T>) {
        if (requestConfig.headers["X-Auth-ID"].isNullOrEmpty()) {
            if (apiKey["X-Auth-ID"] != null) {
                if (apiKeyPrefix["X-Auth-ID"] != null) {
                    requestConfig.headers["X-Auth-ID"] = apiKeyPrefix["X-Auth-ID"]!! + " " + apiKey["X-Auth-ID"]!!
                } else {
                    requestConfig.headers["X-Auth-ID"] = apiKey["X-Auth-ID"]!!
                }
            }
        }
        if (requestConfig.headers["X-Auth-Token"].isNullOrEmpty()) {
            if (apiKey["X-Auth-Token"] != null) {
                if (apiKeyPrefix["X-Auth-Token"] != null) {
                    requestConfig.headers["X-Auth-Token"] = apiKeyPrefix["X-Auth-Token"]!! + " " + apiKey["X-Auth-Token"]!!
                } else {
                    requestConfig.headers["X-Auth-Token"] = apiKey["X-Auth-Token"]!!
                }
            }
        }
    }
